...
> [!NOTE]
> 示例配置中的尾随逗号（例如 `bind = SUPER, Tab, cyclenext,`）表示
> 一个空的 `params` 参数。仅当最后一个参数
> 故意留空时才包含尾随逗号。

```ini
bind = SUPER, F, exec, firefox   # 正确 - 4 个参数
bind = , Print, exec, grim       # 正确 - 4 个参数（第一个为空 = 无修饰键）
bind = SUPER, F, exec, firefox,  # 不正确 - 尝试执行 `firefox,` 但该命令不存在
bind = SUPER, Tab, cyclenext,    # 正确 - 4 个参数（最后一个参数为空（调度器不需要参数））
```

> [!WARNING]
> 意外的尾随逗号会成为参数的一部分（例如 `firefox,` 而不是
> `firefox`）。如果键位绑定不起作用，请检查尾随逗号！

## 非常见符号 / 使用按键码绑定

请参阅
[xkbcommon-keysyms.h 头文件](https://github.com/xkbcommon/libxkbcommon/blob/master/include/xkbcommon/xkbcommon-keysyms.h)
获取所有 keysyms。您应使用的名称是 `XKB_KEY_` 之后的部分。

如果您想通过按键码绑定，可以在 KEY 位置使用
`code:` 前缀，例如：
...
[`sendshortcut`](../Dispatchers/#list-of-dispatchers) 调度器以获取键位绑定。
