...
| exit | 退出合成器，不询问任何问题。建议使用 `hyprshutdown` 代替此命令。 | 无 |
...
> [!WARNING]
> 不建议直接通过键绑定设置 DPMS 或 forceidle，因为这可能会导致未定义行为。相反，考虑使用类似以下的方式
>
> ```ini
...
- 显示器上的工作区，相对值使用 `+` 或 `-`，绝对值使用 `~`：例如 `m+1`、
  `m-2` 或 `m~3`

- 包含空工作区的显示器上的工作区，相对值使用 `+` 或 `-`，
  绝对值使用 `~`：例如 `r+1` 或 `r~3`

- 打开的工作区，相对值使用 `+` 或 `-`，绝对值使用 `~`：例如 `e+1`,
  `e-10`, 或 `e~2`
...
- 第一个可用的空工作区：`empty`, 后缀 `m` 表示仅在显示器上搜索
  和/或 `n` 表示下一个可用的空工作区。例如
  `emptynm`
...
属性是 [窗口规则](../Window-Rules#dynamic-effects) 的任何_动态效果_。
