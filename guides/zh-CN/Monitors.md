...
这将使 `DP-1` 上的显示器成为 `1920x1080` 显示，运行在 144Hz，`0x0` 偏移自左上角，缩放比例为 1（无缩放）。
...
显示器定位在虚拟"布局"上。`position` 是该显示器在布局中的位置，以像素为单位。（从左上角计算）
...
`position` 可以包含_负_值，所以上面的示例也可以写成
...
Hyprland 使用反向 Y 轴笛卡尔坐标系。因此，负 y 坐标会使显示器位置更高，而正 y 坐标会使它位置更低。
...
> [!NOTE]
> 位置是根据缩放（和转换）后的分辨率计算的，这意味着如果您想将 4K 显示器（缩放比例为 2）放在 1080p 显示器的左侧，您会对第二个屏幕使用位置 `1920x0`（3840 / 2）。如果显示器还旋转了 90 度（垂直），您会使用 `1080x0`。

> [!WARNING]
> 显示器不能重叠。这意味着如果您设置的位置使任何显示器重叠，您将收到警告。

> [!NOTE]
> "Invalid scale" 警告将在您的缩放比例无法创建有效的逻辑像素时弹出。有效的缩放比例必须能整除您的分辨率（不带小数）。例如 1920x1080 / 1.5 = 1280x720 -> 正常，但 / 1.4 -> 1371.4286x771.42857 -> 不正常。
...
_**请注意：**_ 虽然允许为第一个显示器指定显示器方向，但这没有效果，它将被定位在 (0,0)。此外，方向总是从中心向外，所以您可以指定 `auto-up` 然后 `auto-left`，但左侧的显示器将仅位于原点左侧并在原点上方。您也可以指定重复的方向，并且显示器将继续沿该方向排列。
...
那么 `description` 值（不包括端口名称 `(eDP-1)`）可以用于指定显示器：
...
> [!WARNING]
> 禁用显示器将字面意义上将其从布局中移除，移动所有窗口和工作区到任何剩余的显示器上。如果您想以屏保样式禁用显示器（仅关闭显示器），请使用 `dpms` [调度器](../Dispatchers)。

## 自定义保留区域

保留区域是平铺窗口不会占用的区域。如果您的工作流程需要自定义保留区域，您可以添加它：

```ini
monitor = 名称, addreserved, TOP, BOTTOM, LEFT, RIGHT
```

其中 `TOP` `BOTTOM` `LEFT` `RIGHT` 是整数，即要添加的保留区域的像素数。这会叠加在计算出的保留区域（例如任务栏）之上，但您在配置中每个显示器只能使用一个此类规则。
...
请记住，镜像显示器不会为第二个显示器"重新渲染"所有内容，所以如果将 1080p 屏幕镜像到 4K 屏幕上，分辨率在 4K 显示器上仍为 1080p。这也意味着在不同宽高比（例如 16:9 和 16:10）上会发生挤压和拉伸。

### 10 位支持

如果您想为显示器启用 10 位支持，在显示器规则末尾添加 `, bitdepth, 10`，例如：
