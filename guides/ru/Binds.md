...
Привязки используют запятые в качестве **разделителей аргументов**. Ключевое слово `bind` ожидает ровно 4 аргумента, поэтому вам нужно ровно 3 запятые:
...
> [!NOTE]
> Завершающие запятые в примерах конфигов (например, `bind = SUPER, Tab, cyclenext,`) указывают
> на пустой аргумент `params`. Включайте завершающую запятую только тогда, когда последний аргумент
> намеренно пуст.

```ini
bind = SUPER, F, exec, firefox   # ОК - 4 аргумента
bind = , Print, exec, grim       # ОК - 4 аргумента (пустой первый = нет модификатора)
bind = SUPER, F, exec, firefox,  # НЕ ОК - пытается выполнить `firefox,`, которого не существует
bind = SUPER, Tab, cyclenext,    # ОК - 4 аргумента (пустой последний аргумент (диспетчеру не нужны параметры))
```

> [!WARNING]
> Случайная завершающая запятая становится частью аргумента (например, `firefox,` вместо
> `firefox`). Если привязка клавиши не работает, проверьте наличие завершающих запятых!
...
Например, французская раскладка [AZERTY](https://ru.wikipedia.org/wiki/AZERTY) использует <key>SHIFT</key> + _`неизменённая клавиша`_ для ввода цифр `0-9`. Таким образом, привязки рабочих пространств для этой раскладки должны использовать имена _`неизменённых клавиш`_, и не будут работать при использовании цифр `0-9`.

> [!NOTE]
> Чтобы получить правильное имя для `неизменённой_клавиши`, обратитесь к [разделу о необычных символах](#uncommon-syms--binding-with-a-keycode)
...
| `m` | mouse | См. специальный раздел [Привязки мыши](#mouse-binds). |
| `t` | transparent | Не может быть перекрыт другими привязками. |
| `i` | ignore mods | Будет игнорировать модификаторы. |
| `s` | separate | Будет произвольно комбинировать клавиши между каждым модификатором/клавишей, см. [Комбинации keysym](#keysym-combos). |
...
| `u` | submap universal | Будет активна независимо от текущей подкарты. |
...
```ini
bind = SUPER, mouse:272, exec, amongus  # привязывает `exec amongus` к SUPER + ЛКМ.
```
...
Вы можете установить привязку клавиши, которая будет активна независимо от текущей подкарты, используя флаг привязки submap universal.
