...
> [!WARNING]
> Правила окон **чувствительны к регистру**. (например, `firefox` ≠
> `Firefox`)
>
...
- `ПРАВИЛО` — это [правило](#rules) (и параметр, если применимо)
- `ПАРАМЕТРЫ` — это разделённый запятыми список различных атрибутов окна, по которым можно выполнять сопоставление. См. поля ниже.
...
Несколько правил могут быть указаны в одной строке, разделённые запятыми. Но они должны сопровождаться хотя бы одним параметром.
...
Статические правила оцениваются один раз при открытии окна и больше никогда. Это по сути означает, что это всегда будут `initialTitle` и `initialClass`, которые будут найдены при сопоставлении по `title` и `class` соответственно.
...
| suppressevent [types...] | Игнорирует определённые события от окна. События разделены пробелом и могут быть: `fullscreen`, `maximize`, `activate`, `activatefocus`, `fullscreenoutput`. |
...
| maxsize [w] [h] | Устанавливает максимальный размер (x,y -> целое число). Применяется к плавающим окнам. (используйте `misc:size_limits_tiled`, чтобы включить плиточные окна.) |
| minsize [w] [h] | Устанавливает минимальный размер (x,y -> целое число). Применяется к плавающим окнам. (используйте `misc:size_limits_tiled`, чтобы включить плиточные окна.) |
...
| dimaround [on] | Затемняет всё вокруг окна. Обратите внимание, что это правило предназначено для плавающих окон, и его использование на плиточных окнах может привести к странному поведению. |
...
| nearestneighbor [on] | Принудительно заставляет окно использовать [ближайший сосед](https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BF%D0%BE%D0%BB%D1%8F%D1%86%D0%B8%D1%8F_%D0%B1%D0%BB%D0%B8%D0%B6%D0%B0%D0%B9%D1%88%D0%B5%D0%B3%D0%BE_%D1%81%D0%BE%D1%81%D0%B5%D0%B4%D0%B0) фильтрацию. |
...
| noscreenshare [on] | Скрывает окно и его всплывающие окна от демонстрации экрана, рисуя чёрные прямоугольники на их месте. Прямоугольники рисуются даже если другие окна находятся сверху. |
...
windowrule = bordercolor rgb(00FF00), fullscreenstate:* 1                 # Установить цвет границы в зелёный, если клиентское состояние полноэкранного режима окна равно 1(максимизация) (внутреннее состояние может быть любым)
windowrule = bordercolor rgb(FFFF00), title:.*Hyprland.*                  # Установить цвет границы в жёлтый, когда заголовок содержит Hyprland
...
Например, если определено правило, которое изменяет `bordercolor` окна, когда оно плавающее, то `bordercolor` изменится на запрошенный цвет, когда оно установлено в плавающее, и вернётся к цвету по умолчанию, когда оно снова будет плиточным.
...
> `activeopacity` в `0.5` и `opacity` в `0.5` приведёт к общей прозрачности
> `0.25`. <br>
