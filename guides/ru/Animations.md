...
`ВКЛ/ВЫКЛ` использует `0` для отключения, `1` для включения. _Примечание:_ если установлено `0`, можно опустить дальнейшие аргументы.

`СКОРОСТЬ` — это количество децисекунд (1 дс = 100 мс), которое займет анимация.

`КРИВАЯ` — это имя кривой Безье, см. [кривые](#curves).

`СТИЛЬ` (необязательный) — стиль анимации.

Анимации образуют дерево. Если анимация не установлена, она унаследует значения родителя. См. [дерево анимаций](#animation-tree).
...
Для стиля анимации `popin` в `windows` можно указать минимальный процент, с которого начнётся анимация. Например, следующая настройка сделает анимацию от 80% до 100% размера:
