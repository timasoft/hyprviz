...
> [!NOTE]
> Пользователям [uwsm](../../Useful-Utilities/Systemd-start) следует избегать размещения переменных окружения в файле `hyprland.conf`.  
...
Вы можете использовать ключевое слово `env` для установки переменных окружения до инициализации сервера отображения, например:
...
- `env = SDL_VIDEODRIVER,wayland` - Запускать приложения SDL2 в Wayland. Удалите или установите в
  `x11`, если игры, предоставляющие более старые версии SDL, вызывают проблемы совместимости.
...
приложения, которые могут устанавливать их за вас, однако это не плохая идея — установить
их явно.

Если ваш [портал рабочего стола](https://wiki.archlinux.org/title/XDG_Desktop_Portal) работает некорректно по, казалось бы,
необъяснимой причине (без ошибок), скорее всего, ваши переменные XDG окружения установлены неправильно.
...
- `env = QT_QPA_PLATFORM,wayland;xcb` - Указывает приложениям Qt использовать бэкенд Wayland
  и переключаться на X11, если Wayland недоступен.
- `env = QT_WAYLAND_DISABLE_WINDOWDECORATION,1` - Отключает декорации окон в приложениях Qt.
- `env = QT_QPA_PLATFORMTHEME,qt5ct` - Указывает приложениям на базе Qt выбирать тему
...
- `GTK_THEME` - Установить тему GTK вручную для тех, кто хочет избежать использования
  инструментов оформления, таких как lxappearance или nwg-look.
- `XCURSOR_THEME` - Установить тему курсора. Тема должна быть установлена и
  доступна для чтения вашим пользователем.
- `XCURSOR_SIZE` - Установить размер курсора. См. [здесь](../../FAQ/) для объяснения,
